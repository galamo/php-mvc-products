...
class DatabaseConnection{
    ...
    public function SelectQuery($q,$fetchBy = ''){
        $resObj = $this->connection->query($q);
        while ($row = $resObj->fetch_object($fetchBy)) {
            $arr[]= $row;
        }
        return $arr;
    }
      ...
      public function InsertQuery($query){
          ...
          $resObj = $this->connection->query($query) or die(mysqli_error($this->connection));
          if($resObj) return $this->connection->insert_id;
          return false;
